...
│   └── app.py             # [将来] Streamlit Webアプリのエントリーポイント
...
* **生データ保存ポリシー**:
  * CSVファイルには **DATファイルの全列をそのまま保存** し、加工済みの列（`timestamp`, `volume_m3` 等）は一切持たない方針としています。
...
* **ローカルDB（CSV）への差分合成**:
  ...
  * この際、日付(0列目)と時刻(1列目)をキーに重複を排除し、新データを優先して上書き合成する設計としています。

## 4. Web UI の設計方針

### 採用技術: Streamlit + Streamlit Community Cloud
フェーズ3のWebアプリケーション化には **Streamlit** を採用します。

**選定理由:**
* Pythonのみで完結し、HTML/CSS/JavaScriptの知識が不要。
* 既存の `matplotlib` グラフを `st.pyplot(fig)` に置き換えるだけで移行可能。
...
**不採用とした代替案:**
* **FastAPI**: API層としては優秀だが、フロントエンド（HTML/JS）を別途構築する必要があり、ダッシュボード用途にはオーバースペック。
* **Vercel**: フロントエンド特化で、Pythonのmatplotlib等の重い描画ライブラリとの相性が悪い。
