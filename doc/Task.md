...
- [x] **Step 1.5: 表示期間選択UIの導入**
  - `storage.py` に `DisplayPeriod` enum（`TWO_WEEKS` / `ONE_YEAR` / `ALL`）と `_period_to_since()` を追加。
  - `load_data()` が `period: DisplayPeriod` を受け取り、DBクエリレベルで期間フィルタ（`.gte`）をかける。
  - `app.py`: ラジオボタン（2週間 / 1年 / 全部、デフォルト: 2週間）を追加。
  - `main.py`: `DISPLAY_PERIOD` 定数 + `--period` CLI引数を追加。
- [ ] **Step 2: 表示期間に応じた動的集約**
  - 表示期間に応じて雨量の集約間隔を自動調整する。
  - 目安: ~2週間→1時間、~一年→1日、~3ヶ月以上→1日。
