...
# __file__ は tests/conftest.py に位置するため、2階層上がプロジェクトルート
    ...
    def side_effect(url, *args, **kwargs):
        ...
        if url.endswith(".dat"):
            if "5313680" in url:  # miyagase_dam 向けリンクという想定 (適当な識別子)
                with open(os.path.join(fixtures_dir, "miyagase_dam.dat"), "r", encoding="utf-8") as f:
                    # Windows特有の \r\n -> \r\r\n の連続改行を吸収する
                    text = f.read().replace('\n\n', '\n')
                    return MockResponse(text)
            else:
                with open(os.path.join(fixtures_dir, "miyagase_rain.dat"), "r", encoding="utf-8") as f:
                    text = f.read().replace('\n\n', '\n')
                    return MockResponse(text)
...
def mock_supabase():
    ...
    class MockTable:
        ...
        def upsert(self, records):
            ...
            # NaNをNoneに変換してから格納する（実際のSupabaseクライアントも似た挙動をする）
            clean_records = []
            for r in records:
                clean_r = {}
                for k, v in r.items():
